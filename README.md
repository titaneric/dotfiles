...
APM-managed agent skills are declared under:

```text
dot_apm/apm.yml.tmpl -> ~/.apm/apm.yml -> ~/.agents/skills
```

`dot_agents/skills` is excluded from chezmoi management. On the first apply after
this migration, the one-time cleanup script removes the existing
`~/.agents/skills` directory before installers rebuild it.

APM keeps its generated lockfile and package cache in `~/.apm/`. The lockfile is
machine-local because it can include private repository metadata. Public skills
are declared by `dot_apm/apm.yml.tmpl`; private repositories are added with
structured TOML in ignored `.chezmoidata/agents.toml`:
...
After the APM manifest is rendered, `.chezmoiscripts/run_after_40-install-apm-agent-skills.sh.tmpl` bootstraps or replays the global skill graph. It creates `~/.apm/apm.lock.yaml` on the first apply and uses frozen installation on every later apply:
...
To intentionally refresh skills, review the resulting local lockfile, then
...
| `.chezmoiscripts/run_once_after_34-reset-agent-skills.sh.tmpl` | Removes legacy `~/.agents/skills` once so installers can take ownership. |
| `.chezmoiscripts/run_after_35-install-agent-skills.sh.tmpl` | Refreshes agent skills from installed CLIs when available. |
| `.chezmoiscripts/run_after_40-install-apm-agent-skills.sh.tmpl` | Bootstraps and frozen-replays the global APM skill graph. |
